class Video {
  ...
  displayVideo() {
    ...
    melancholy.removeAttribute("hidden");
            ...
            text(`Video Page`, windowWidth/2, windowHeight/2 + screenPushAmount);
            pop();
    ...
    menuTextDetect(){
      if(state === `video`){
      if(mouseX < windowWidth - 178 && mouseX > windowWidth - 265 && mouseY > 42 + screenPushAmount && mouseY < 82 + screenPushAmount){
    state = `music`;
    menuToggle = 0;
      }
      if(mouseX < windowWidth - 152 && mouseX > windowWidth - 265 && mouseY > 100 + screenPushAmount && mouseY < 140 + screenPushAmount){
        state = `drawing`;
        menuToggle = 0;
          }
